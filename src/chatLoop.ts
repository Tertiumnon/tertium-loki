...
import { chatStream, listModels, type ChatMessage } from "./ollamaClient.js";
import { runSetupWizard } from "./setupWizard.js";
import type { AgentProfile, Config } from "./config.js";

const RESET = "\x1b[0m";
const GRAY = "\x1b[90m";
const BLUE = "\x1b[94m";

function printHelp(config: Config): void {
...
export async function runChatLoop(config: Config): Promise<void> {
  ...
  let agent: AgentProfile =
    config.agents.find((a) => a.name === config.defaultAgent) ?? config.agents[0];
  ...
  try {
    while (true) {
      ...
      const outgoing: ChatMessage[] = agent.systemPrompt
