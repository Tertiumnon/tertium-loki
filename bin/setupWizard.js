...
import { listModelsDetailed } from "./ollamaClient.js";
import { ROLE_ORDER, ROLE_DEFAULT_PROMPTS, describeModel, isChatCapable, suggestForRole, } from "./modelSuggest.js";
...
export async function runSetupWizard(existingRl) {
    ...
    try {
        ...
        const agents = [];
        const suggestions = ROLE_ORDER.map((role) => ({ role, model: suggestForRole(chatModels, role) })).filter((s) => s.model !== undefined);
